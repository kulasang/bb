...
cl = LINETCR.LINE()
cl.login(token="Eqp32xlZvz05OjrIEds3.yTMpGHq0GPo0CSFgVHZsKW.F6CKtng+7pkUksQBvMm3LvbkDbPcO7f5If1OZhcQho8=")
cl.loginResult()
...
✪ Getmid @ ↬ (ดูเอ็มไอดีคนอื่น)
✪ Getname @ ↬ (ดูชื่อคนอื่น)
✪ Spam @ ↬ (สแปมคนอื่น)
✪ Mimic on ↬ (เปิดเลียนคำพูด)
✪ Mimic off ↬ (ปิดเลียนคำพูด)
...
✪ Command Setting ★
═╬═══════════
✿ Add on ↬ (เปิดทักก่อนรับแอด)
✿ Add off ↬ (ปิดทักก่อนรับแอด)
✿ Contact on ↬ (เปิดอ่าน คท.)
✿ Contact off ↬ (ปิดอ่าน คท.)
✿ Join on ↬ (เปิดเข้ากลุ่มอัตโนมัติ)
✿ Join off ↬ (ปิดเข้ากลุ่มอัตโนมัติ)
✿ Leave on ↬ (เปิดกันรันแชทรวม)
✿ Leave off ↬ (ปิดกันรันแชทรวม)
✿ Like on ↬ (เปิดไลค์อัตโนมัติ)
✿ Like off ↬ (ปิดไลค์อัตโนมัติ)
✿ Link on ↬ (เปิดลิงค์กลุ่ม)
✿ Link off ↬ (ปิดลิงค์กลุ่ม)
✿ Ment on ↬ (เปิดคอมเมนต์)
✿ Ment off ↬ (ปิดคอมเมนต์)
✿ Share on ↬ (เปิดแชร์ไทม์ไลน์)
✿ Share off ↬ (ปิดแชร์ไทม์ไลน์)
✿ Wc on ↬ (เปิดข้อความต้อนรับ)
✿ Wc off ↬ (ปิดข้อความต้อนรับ)
✿ Tag on ↬ (เปิดตอบแท็กอัตโนมัติ)
✿ Tag off ↬ (ปิดตอบแท็กอัตโนมัติ)
✿ Tagkick on ↬ (เปิดเตะคนแท็ก)
✿ Tagkick off ↬ (ปิดเตะคนแท็ก)
✿ Set ↬ (ดูการตั้งค่า)

═╬═══════════
✿ Command Guard ★
═╬═══════════
✪ Allguard on ↬ (เปิดป้องกันทั้งหมด)
✪ Allguard off ↬ (ปิดป้องกันทั้งหมด)
✪ Cancel on ↬ (เปิดยกเลิกคำเชิญ)
✪ Cancel off ↬ (ปิดยกเลิกคำเชิญ)
✪ Guard on ↬ (เปิดบอทเตะทำงาน)
✪ Guard off ↬ (ปิดบอทเตะทำงาน)
✪ Invite on ↬ (เปิดให้เชิญเพื่อน)
✪ Invite off ↬ (ปิดให้เชิญเพื่อน)
✪ Qr link on ↬ (เปิดป้องกันลิงค์กลุ่ม)
✪ Qr link off ↬ (ปิดป้องกันลิงค์กลุ่ม)
...
Bots = [mid,kimid,ki2mid,ki3mid,ki4mid,ki5mid,"u62d98f8214348293b472c7c6be1cd293"]
bot1 = cl.getProfile().mid
admsa = "u62d98f8214348293b472c7c6be1cd293"
admin = "u62d98f8214348293b472c7c6be1cd293"
creator = "u62d98f8214348293b472c7c6be1cd293"
...
wait = {
    ...
    'timeline':False,
    'autoAdd':True,
    'message':"ขอบคุณที่เพิ่มฉันเป็นเพื่อน ผมชื่อ LRC♡ เหน่งนะ",
    "lang":"JP",
    "comment":"ข้าคือมือพระกาฬ LRC♡ เหน่ง",
    "commentOn":False,
    "likeOn":False,
    'detectMention':True,	
    'kickMention':False,  	
    ...
    "inviteprotect":False,
    "linkprotect":False,
    "welmsg":"ยินดีต้อนรับสมาชิกใหม่ครับ",	
    "welcomemsg":True,	
}
...
def bot(op):
    ...
    try:
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                msg.to = msg.from_
                if msg.from_ == "u62d98f8214348293b472c7c6be1cd293":
                    if "join:" in msg.text:
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     contact = cl.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["สวัสดีครับคุณ " + cName + str("\nมีอะไรให้รับใช้ไหมครับ") , "" + cName + str(" แท็กได้แท็กดี") , "" + cName + str(" จะแท็กอะไรกันนักหนา")]										
                     ret_ = random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
...
#			    random.choice(KAK).kickoutFromGroup(op.param1,[op.param2])
	   ...
	   if wait["welcomemsg"] == True:
        ...
        if op.type == 15:
          if op.param2 in Bots:
             return
          random.choice(KAC).sendText(op.param1, "เธอไปไม่ลาสักคำไม่ทันได้เตรียมใจ")
